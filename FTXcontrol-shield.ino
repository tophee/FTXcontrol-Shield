...
void setup() {
  ...
  if (ventstate < 3){desiredventstate = ventstate;}         // make sure ventstate wont be changed at every reset, but only if led reading succeeded
  if (ventstate = 2){autohigh = true;}                      // to avoid getting stuck in high ventilation, we assume it was set automatically (giving permission to change it)
...
void pin_ISR() {                                            // note that the interrupt pin has to be pin 2 or 3
  buttonpress = 1;
...
int ledventstate(int x){
  ...
  if (x > 10 && x < 20){ // values with 10k resistior: 800 - 900
    lederror = 0;
    DEBUG_PRINT((F("Status: Current ledventstate is 0")));
    lastledventstate = 0;
    return 0;
   }
   else if (x > 25 && x < 60){ // values with 10k resistior: 600 - 760
    lederror = 0;
    DEBUG_PRINT((F("Current ledventstate is 1")));
    lastledventstate = 1;
    return 1;
   }
   else if (x > 80 && x < 200){    // values with 10k resistior: 280 - 400
    lederror = 0;
...
void send_data(){
  ...
  switch (printmode){
    ...
    case 2:
	  ...
	  print_csv(F(" brightness"), ledbrightness);                      
